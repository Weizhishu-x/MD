...
def resume_and_load_MAE(model, ckpt_path, device):
    print("Loading checkpoints for MAE from", ckpt_path)
    checkpoints = torch.load(ckpt_path, map_location=device)
    mae_decoder_weights = {k.replace('MAEDecoder.', ''): v 
                      for k, v in checkpoints.items() 
                      if k.startswith('MAEDecoder.')}
    missing_keys, unexpected_keys = model.load_state_dict(mae_decoder_weights, strict=True)
    print("MAEDecoder Missing keys:", missing_keys)
    print("MAEDecoder Unexpected keys:", unexpected_keys)
    return model
...
def selective_reinitialize(model, reinit_ckpt, modules):
