...
class CocoStyleDataset(CocoDetection):
    ...
    img_dirs = {
        'xView': {
            'train': '/input0/xView/images',
        },
        'DOTA': {
            'train': '/input0/DOTA/train/images', 
            'val': '/input0/DOTA/val/images',
        },
        'HRRSD': {
            'train': '/input1/HRRSD2SSDD/HRRSD/images',
        },
        'SSDD': {
            'train': '/input1/HRRSD2SSDD/SSDD/images/train',
            'val': '/input1/HRRSD2SSDD/SSDD/images/test',
        },
    }
    anno_files = {
        'xView': {
            'source': {
                'train': '/input0/xView/annotations/train_3c.json',
            },
        },
        'DOTA': {
            'target': {
                'train': '/input0/DOTA/annotations/train_3c.json',
                'val': '/input0/DOTA/annotations/val_3c.json',
            }
        },
        'HRRSD': {
            'source': {
                'train': '/input1/HRRSD2SSDD/HRRSD/annotations/train_ship_resized.json',
            },
        },
        'SSDD': {
            'target': {
                'train': '/input1/HRRSD2SSDD/SSDD/annotations/train_resized.json',
                'val': '/input1/HRRSD2SSDD/SSDD/annotations/test_resized.json',
            }
        },
    }
    ...
    def collate_fn(batch):
        ...
        # batched_images = CocoStyleDataset.pad_mask(image_list)
        # annotations = [sample[1] for sample in batch]
        # return batched_images, annotations
...
class CocoStyleDatasetTeaching(CocoStyleDataset):
    ...
    def collate_fn_teaching(batch):
        ...
        assert batch[0].mask.equal(batch[1].mask)
...
class DataPreFetcher:
    ...
    def to_cuda(self):
        try:
            self.next_samples = self.next_samples.to(self.device, non_blocking=True)
        except:
            self.next_samples = [sample.to(self.device, non_blocking=True) for sample in self.next_samples]
    ...
    def preload(self):
        try:
            ...
            self.next_samples, self.next_annotations = next(self.loader)
    ...
    def next(self):
        ...
        samples, annotations = self.next_samples, self.next_annotations
        if samples is not None:
            try:
                self.next_samples.record_stream(torch.cuda.current_stream())
            except:
                for sample in self.next_samples:
